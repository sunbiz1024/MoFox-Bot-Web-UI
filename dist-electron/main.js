...
async function saveLastDirectory(directoryPath) {
  try {
    await require$$1__namespace.promises.writeFile(CONFIG_PATH, JSON.stringify({ lastDirectory: directoryPath }));
  } catch (error) {
...
async function getLastDirectory() {
  try {
    if (await require$$1__namespace.promises.access(CONFIG_PATH).then(() => true).catch(() => false)) {
      const content = await require$$1__namespace.promises.readFile(CONFIG_PATH, "utf8");
...
electron.app.whenReady().then(() => {
  ...
  electron.ipcMain.handle("get-system-stats", async () => {
    var _a, _b;
    console.log("[main.ts] Received get-system-stats request");
    try {
      const cpuUsage = await libExports.currentLoad();
      const mem = await libExports.mem();
      const gpu = await libExports.graphics();
      const fsSize = await libExports.fsSize();
      const stats = {
        cpu: cpuUsage.currentLoad,
        memory: mem.active / mem.total * 100,
        gpu: ((_a = gpu.controllers[0]) == null ? void 0 : _a.utilizationGpu) || 0,
        disk: ((_b = fsSize[0]) == null ? void 0 : _b.use) || 0
      };
      console.log("[main.ts] Sending system stats:", stats);
      return stats;
    } catch (error) {
      console.error("[main.ts] Failed to get system stats:", error);
      return null;
    }
  });
  createWindow();
